...
という点である。「大前提1 - SSoTは実装」の思想と直結しており、「AIが記憶を捏造していないか」という懐疑への最も強い反証になる。
...
- **テスト手法と採用理由**: 単体テスト110件に加え、本番C#実装と独立再実装したPythonロジックを実データ（約38万件のVitalSign）に走らせる「DJC回帰テスト」を採用した理由として、**入眠時刻には外部正解ラベルが存在せずAccuracy的なKPIを作るとGoodhartの法則で指標自体が歪む**、という設計判断の背景まで言語化している。

つまりこれは「コードを読んで動作を要約した」のではなく、**git履歴・Issue番号・コミットハッシュ・テストデータの出所（患者名・件数）まで横断して、設計判断の"なぜ"を再構成した**例である。「大前提1 - SSoTは実装」「大前提3 - DJC」が、単なる思想ではなく実際にリポジトリから読み出し可能な形で埋め込まれていることの証拠になっている。
